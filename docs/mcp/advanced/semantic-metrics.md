...
!!! note "Most metrics are auto-generated candidates"
    The registry carries ~1,037 metrics, but only ~72 are `approved`. The
    rest are `candidate`s auto-generated by `dbt-cerebro`'s
    `scaffold_metrics.py` — one per eligible measure — so every measure is
    reachable by name. `discover_metrics` only surfaces `approved`
    metrics, so this scale does **not** add discovery noise: you'll see the
    curated set, and reach a candidate by exact name via
    `get_metric_details` or by passing `allow_candidate=true` to
    `query_metrics`. Treat candidate numbers as drafts until an analyst
    promotes them.
