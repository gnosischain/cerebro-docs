...
Backed by `dbt-cerebro` semantic YAML; the registry is reloaded periodically and metrics are tagged by quality tier (`approved` / `candidate` / `blocked`).

!!! note "Approved vs candidate metrics"
    The registry carries 1,645 metrics: 1,383 `approved` and 262
    `candidate`s auto-generated by `dbt-cerebro`'s `scaffold_metrics.py`
    — one per eligible measure — so every measure is reachable by name.
    `discover_metrics` only surfaces `approved` metrics, so candidates do
    **not** add discovery noise: you'll see the curated set, and reach a
    candidate by exact name via `get_metric_details` or by passing
    `allow_candidate=true` to `query_metrics`. Treat candidate numbers as
    drafts until an analyst promotes them.
...
- [Tools — Semantic](../tools.md#semantic) — `query_metrics`; [Tools — Analytics](../tools.md#analytics) — `execute_query`
